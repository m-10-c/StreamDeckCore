...
public class StreamDeck implements InputReportListener, IStreamDeck {
     ...
     * Reset command
     */
    private static final byte[] RESET_DATA = new byte[]{0x03, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     ...
     */
    private static final byte[] PAGE_1_HEADER = new byte[]{0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    /**
     * Header for Page 2 of the image command
     */
    private static final byte[] PAGE_2_HEADER = new byte[]{0x02, 0x07, 0x00, 0x01, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    public synchronized void internalDrawImage(int keyIndex, SDImage imgData) {
        ...
        int pageLength = reportLength - PAGE_1_HEADER.length - 1;
        int pages = (int) Math.ceil((float) imgData.sdImage.length / pageLength);
        int totalBytes = imgData.sdImage.length;
        for (int i = 0; i < pages; i++) {
            byte[] page = Arrays.copyOfRange(imgData.sdImage, i * pageLength, Math.min(((i + 1) * pageLength), imgData.sdImage.length));
            totalBytes -= totalBytes;
            byte[] report = new byte[reportLength];
            for (int i1 = 0; i1 < PAGE_1_HEADER.length; i1++) {
                report[i1] = PAGE_1_HEADER[i1];
            }
            for (int j = 0; j < page.length; j++) {
                report[PAGE_1_HEADER.length + j] = page[j];
            }
            report[1] = (byte) keyIndex;
            report[2] = i < pages - 1 ? (byte) 0x00 : (byte) 0x01;
            report[3] = (byte) (page.length & 0xff);
            report[4] = (byte) ((page.length >> 8) & 0xff);
            report[5] = (byte) (i & 0xff);
            report[6] = (byte) ((i >> 8) & 0xff);

            int result = this.hidDevice.setOutputReport((byte) 0x02, report, report.length - 1);
            System.out.println("key " + keyIndex + ", frame " + i + " => " + result);
            if (result < 0) {
                break;
            }
        }
     ...
     *
     * @param keyId   Id of the key to be updated
     * @param imgData image data in the bgr-format
    ...
    private byte[] generatePage1(int keyId, byte[] imgData) {
        ...
        if (imgData != null) {
            for (int i = 0; i < imgData.length && i < NUM_FIRST_PAGE_PIXELS * 3 && PAGE_1_HEADER.length + i < p1.length - 1; i++) {
